...
const Input = forwardRef<HTMLInputElement, InputProps>(({ errorMessage, label, fullWidth = false, size = 40, type = "text", className, ...props }, ref: ForwardedRef<HTMLInputElement>) => {
  ...
  return (
    <div className={cn("w-full min-w-[200px]", fullWidth ? "max-w-full" : "w-full")}>
      <div className="relative">
        ...
        {type === "password" && (
          <button type="button" onClick={() => setShowPassword((prev) => !prev)} className="absolute top-[3.3rem] left-2 -translate-y-1/2 transform text-gray-500">
